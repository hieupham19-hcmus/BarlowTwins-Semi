...
class ContextMasking(nn.Module):
    ...
    def forward(self, x, pred=None):
        x_masked_context = guided_masking(x, pred, resize=(x.size(2)), return_msk_context=True)
        return x_masked_context
...
class ObjectMasking(nn.Module):
    ...
    def forward(self, x, pred=None):
        x_masked_obj = guided_masking(x, pred, resize=(x.size(2)), return_msk_context=False)
        return x_masked_obj
#--------------------------------------------------------------------------------------
#--------------------------------------------------------------------------------------
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # build encoder and bottleneck layers
        self.main_decoder = Decoder(img_size, patch_size, in_chans, num_classes, embed_dim, depths, depths_decoder,
        ...
        self.aux1_decoder = Decoder(img_size, patch_size, in_chans, num_classes, embed_dim, depths, depths_decoder,
                                    num_heads, window_size, mlp_ratio, qkv_bias, qk_scale, drop_rate,
                                    attn_drop_rate, drop_path_rate, norm_layer, ape, patch_norm, use_checkpoint, final_upsample,
                                    patches_resolution, dpr)
        self.aux2_decoder = Decoder(img_size, patch_size, in_chans, num_classes, embed_dim, depths, depths_decoder,
                                    num_heads, window_size, mlp_ratio, qkv_bias, qk_scale, drop_rate,
                                    attn_drop_rate, drop_path_rate, norm_layer, ape, patch_norm, use_checkpoint, final_upsample,
                                    patches_resolution, dpr)
        self.aux3_decoder = Decoder(img_size, patch_size, in_chans, num_classes, embed_dim, depths, depths_decoder,
                                    num_heads, window_size, mlp_ratio, qkv_bias, qk_scale, drop_rate,
                                    attn_drop_rate, drop_path_rate, norm_layer, ape, patch_norm, use_checkpoint, final_upsample,
                                    patches_resolution, dpr)
        self.aux4_decoder = Decoder(img_size, patch_size, in_chans, num_classes, embed_dim, depths, depths_decoder,
                                    num_heads, window_size, mlp_ratio, qkv_bias, qk_scale, drop_rate,
                                    attn_drop_rate, drop_path_rate, norm_layer, ape, patch_norm, use_checkpoint, final_upsample,
                                    patches_resolution, dpr)
        self.aux5_decoder = Decoder(img_size, patch_size, in_chans, num_classes, embed_dim, depths, depths_decoder,
                                    num_heads, window_size, mlp_ratio, qkv_bias, qk_scale, drop_rate,
                                    attn_drop_rate, drop_path_rate, norm_layer, ape, patch_norm, use_checkpoint, final_upsample,
                                    patches_resolution, dpr)
        self.aux6_decoder = Decoder(img_size, patch_size, in_chans, num_classes, embed_dim, depths, depths_decoder,
                                    num_heads, window_size, mlp_ratio, qkv_bias, qk_scale, drop_rate,
                                    attn_drop_rate, drop_path_rate, norm_layer, ape, patch_norm, use_checkpoint, final_upsample,
                                    patches_resolution, dpr)
        self.aux7_decoder = Decoder(img_size, patch_size, in_chans, num_classes, embed_dim, depths, depths_decoder,
                                    num_heads, window_size, mlp_ratio, qkv_bias, qk_scale, drop_rate,
                                    attn_drop_rate, drop_path_rate, norm_layer, ape, patch_norm, use_checkpoint, final_upsample,
                                    patches_resolution, dpr)
    ...
    def forward(self, x):
        ...
        main_seg = self.main_decoder(x,x_downsample)
        
        aux1_feature = [FeatureNoise()(i) for i in x_downsample]
        aux1 = FeatureNoise()(x)
        aux1_seg = self.aux1_decoder(aux1, aux1_feature)
        
        aux2_feature = [Dropout(i) for i in x_downsample]
        aux2 = Dropout(x)
        aux2_seg = self.aux2_decoder(aux2, aux2_feature)
        
        aux3_feature = [FeatureDropout(i) for i in x_downsample]
        aux3 = FeatureDropout(x)
        aux3_seg = self.aux3_decoder(aux3, aux3_feature)
        
        aux4_feature = [CutOut()(i, main_seg) for i in x_downsample]
        aux4 = CutOut()(x, main_seg)
        aux4_seg = self.aux6_decoder(aux4, aux4_feature)
        
        aux5_feature = [VAT()(i, self.aux5_decoder) for i in x_downsample]
        aux5 = VAT()(x, self.aux5_decoder)
        aux5_seg = self.aux7_decoder(aux5, aux5_feature)
        
        aux6_feature = [ContextMasking()(i, main_seg) for i in x_downsample]
        aux6 = ContextMasking()(x, main_seg)
        aux6_seg = self.aux6_decoder(aux6, aux6_feature)
        
        aux7_feature = [ObjectMasking()(i, main_seg) for i in x_downsample]
        aux7 = ObjectMasking()(x, main_seg)
        aux7_seg = self.aux7_decoder(aux7, aux7_feature)

        return main_seg, aux1_seg, aux2_seg, aux3_seg, aux4_seg, aux5_seg, aux6_seg, aux7_seg
